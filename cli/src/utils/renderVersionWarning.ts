...
export const renderVersionWarning = (npmVersion: string) => {
  ...
  if (currentVersion.includes("beta")) {
    logger.warn("  You are using a beta version of create-t3-fire.");
    logger.warn("  Please report any bugs you encounter.");
  } else if (currentVersion.includes("next")) {
    logger.warn(
      "  You are running create-t3-fire with the @next tag which is no longer maintained."
    );
    logger.warn("  Please run the CLI with @latest instead.");
  } else if (npmVersion && currentVersion !== npmVersion) {
    logger.warn("  You are using an outdated version of create-t3-fire.");
    logger.warn(
      "  Your version:",
      currentVersion + ".",
...
function checkForLatestVersion(): Promise<string> {
  return new Promise((resolve, reject) => {
    https
      .get(
        "https://registry.npmjs.org/-/package/create-t3-fire/dist-tags",
        (res) => {
...
export const getNpmVersion = () =>
  ...
  checkForLatestVersion().catch(() => {
    try {
      return execSync("npm view create-t3-fire version").toString().trim();
    } catch {
      return null;
    }
  });
